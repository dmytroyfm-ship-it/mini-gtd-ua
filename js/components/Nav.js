...
// одразу перед іконкою акаунта. Авто-відступ (margin-left: auto) —
// лише на .nav__panel: він штовхає праворуч ЦІЛУ групу, що йде після
// нього (розділювач + лупа + .nav__account-slot) як єдиний блок.
// Двох незалежних auto-margin (раніше й на .nav__panel, і на
// .nav__account-slot) навмисно уникаємо — CSS ділить вільний простір
// порівну між УСІМА такими краями на одному ряду, тож два auto-margin
// давали два окремі розтягнуті проміжки замість одного. На мобільному
// .nav__panel випадає з потоку (position: fixed), тож .nav__search
// там отримує свій власний margin-left: auto (лишається сам, без
// конкурента) — інакше лупа лишалась би зліва біля бренду; сам
// розділювач перед лупою на мобільному ховається (сенсу нема, коли
// посилання все одно сховані в бургер-меню).
//
