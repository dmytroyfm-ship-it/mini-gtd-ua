...
(наприклад, 30-е в 30-денному вересні) — зберігається сентинел `31`
("завжди останній день місяця"), і `Math.min(31, довжина місяця)` сам
щоразу дає справжній останній день (30/31/28/29) замість буквальної
"30". `completeTask()` копіює `recurrence_anchor_day` в нову задачу
незмінним — той самий орієнтир на весь час дії повторення.
...
змішані в один список. Обидва розділи діляться тим самим набором
...
— цей місяць, `весь` — без обмежень, чи довільний `ДД-ММ-РРРР
ДД-ММ-РРРР`. Без inline-клавіатури (бот більше ніде її не
