...
- **«Джерела» / «Стрічка»** — активно працює. Приймальна частина
  (SQL-міграція, `FEED_WEBHOOK_SECRET`, деплой `feed-webhook`) і
  автоматичний збір (`feed-poll` + його `pg_cron`-міграція, розділ 1,
  підрозділ «"Стрічка" — автоматичний збір») підтверджені живими
  тестами 2026-08-31: YouTube/Telegram/RSS — 32/32 постів збережено.
  Instagram — через Apify (`APIFY_API_TOKEN`), лишається задати
  секрет і перевірити на реальному джерелі. Threads/Reddit/Twitter —
  і далі лише вручну/через сторонній парсер (нема безкоштовного RSS
  і не підключено Apify-актора для них).
