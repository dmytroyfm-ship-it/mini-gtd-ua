...
[HistoryList.js](../js/components/HistoryList.js)) — два запити
(`getArchivedTasks()` + `subtaskStore.getSubtasksByTaskIds()` для
всіх їхніх id одразу), далі все на клієнті (обсяг для особистого
...
- **Розклад підзадач** під рядком (лише коли підзадачі є) —
  ...
  `✓` і закреслені, решта з `○`. Лише для огляду (звіт, не робочий
  екран) — без чекбоксів; редагувати підзадачі можна на `/task/:id`.
