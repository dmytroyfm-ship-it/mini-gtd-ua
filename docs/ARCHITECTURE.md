...
**Пошук** (`js/pages/search.js`, `/search`) — за словом (назва/
нотатка) чи тегом, `taskStore.searchTasks()`: один запит по всіх
активних задачах одразу (включно з «Історією»), фільтрація на
клієнті (для особистого обсягу простіше й безпечніше за
ILIKE/OR-фільтр з ризиком зламати запит спецсимволом). Поле — в
`Nav.js`, завжди на видноті; маршрутизація (`js/router.js`) не
підтримує query-рядки, тож запит передається через
`setPendingSearchQuery()` перед `navigate("/search")`, а не
`?q=...`. Результати — ті самі `TaskCard.js`/`TaskList.js`, що й
скрізь, з повним набором дій.
