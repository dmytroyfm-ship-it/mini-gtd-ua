...
- **`subtasks`** — підзадачі (Next Actions): `id`, `task_id`,
  ...
  `tags` із третьої міграції нижче, плюс `position` із
  [20260901010000_add_position_to_subtasks.sql](../supabase/migrations/20260901010000_add_position_to_subtasks.sql)
  (явний порядок — кнопки «↑»/«↓» у рядку підзадачі, лише в
  detailed-режимі; `created_at` лишається тай-брейком). Підключено —
  [js/store/subtaskStore.js](../js/store/subtaskStore.js)
  (`getSubtasks`, `getSubtasksByTaskIds` — батч для «Історії»,
  `addSubtask` — нова йде в кінець (`max(position)+1`),
  `setSubtaskCompleted`, `setSubtaskDueDate`, `setSubtaskTags`,
  `setSubtaskPositions` — новий порядок як масив id, `deleteSubtask`)
  і UI в картці задачі та на сторінці `/task/:id` (розділ 3).
...
«Задачі», дошка) підзадача лишається простим чекліст-пунктом. Так
само detailed-only — кнопки **«↑»/«↓»** (зміна порядку): `SubtaskList`
переставляє рядки й свою робочу копію одразу (оптимістично), тоді
кличе `onReorder(orderedIds)` → `subtaskStore.setSubtaskPositions()`
(окремий UPDATE кожній підзадачі); при помилці `TaskCardSubtasks`
перечитує список, щоб DOM не розійшовся з базою. У detailed-режимі
кнопки дій підзадачі (`✎ ✕ ↑ ↓`) видимі **завжди** (не лише на
`:hover`, як у компактних картках) — інакше нову фічу ↑/↓ було не
знайти, а на дотику `:hover` не існує (звіт користувача). На
мобільному (`max-width: 560px`) detailed-рядок ще й переноситься:
дедлайн на всю ширину, тоді «↑↓✎✕» окремим рядком.
...
орієнтир на весь час дії повторення. `setTaskRecurrence()` чистить
`recurrence_anchor_day` назад до `null` при переході на не-`monthly`
(анкер має сенс лише для monthly), але `recurrence_window_days`
**не чіпає** — період тепер доступний для будь-якої задачі (див.
нижче), тож ним керує лише сам користувач (поле «Початок періоду»)
і `setTaskDueDate(null)` (прибрати дедлайн = прибрати й період).

**Період дедлайну** — необов'язковий, доступний для **будь-якої
задачі** (не лише `weekly`/`monthly` — за прямим проханням
користувача): дедлайн (`due_date`) — кінець періоду, поле «Початок
періоду» («З») поруч — початок; порожнє = звичайний одноденний
дедлайн. Для повторюваної задачі період їде разом із нею на
наступний цикл; для неповторюваної це «тіклер»: задача не
показується на дошці, поки не настане початок (`isWindowActive()`).
Зберігається не сама дата початку, а довжина періоду в днях —
...
`getAllTasks()` (дошка `/board`) віддає **лише `list: "next"`**
(«Задачі») — за прямим проханням користувача: дошка це вужчий фокус
на тому, що вже розписано в роботу. «Вхідні» (ще не розібране),
«Колись» (свідомо відкладене), «Читати/Дивитись» (матеріали) та
«Історія» (вже закрите, автоперенесене о 22:30) на дошку не
потрапляють. Цим же покрито й зворотний бік: перенос задачі в
інший список (dropdown у картці) прибирає її з дошки на наступному
`refreshBoard()` — окремої логіки не треба.
...
лише один було б недостатньо). Колонка «В очікуванні» на дошці
показує задачі статусу `"waiting"` **лише зі списку «Задачі»** (як і
решта колонок — `getAllTasks()` фільтрує за `list: "next"`); задачі
`"waiting"` з інших списків видно на сторінці «Колись», не на дошці.
...
[HistoryList.js](../js/components/HistoryList.js)) — читає задачі +
їхні підзадачі одним `subtaskStore.getSubtasksByTaskIds()`, далі все
на клієнті (обсяг для особистого використання не той, щоб
виправдовувати перезапит на кожну зміну періоду):

- **Перемикач режиму** «Усі» / «Тільки завершені» (вибір у
  `localStorage`, ключ `mini-gtd-history-scope`, дефолт — «Тільки
  завершені», щоб зберегти звичний сенс «Історії»):
  - *Тільки завершені* — `getArchivedTasks()`: виконані/скасовані/
    архів, фільтруються за періодом.
  - *Усі* — додатково `getActiveTasksWithDoneSubtasks()`: **активні**
    задачі, у яких є хоч одна виконана підзадача (задача без жодної
    виконаної підзадачі не показується — прохання користувача). Це
    «бачити, що я вже зробив, а що лишилось» по незавершених. Ці
    рядки НЕ залежать від вибраного періоду (поточний стан, не подія
    в минулому) і йдуть зверху списку.
- **Позначка** на кожному рядку — `✅ Виконано` / `🚫 Скасовано` /
  `🔵 В роботі` (активна, режим «Усі» — без дати, бо `updated_at` тут
  = «коли редагував задачу», не «коли робив підзадачі») / `📁 В
  архіві` (старі задачі, занесені сюди ще вручну — ні виконані, ні
  скасовані). Кнопка «Повернути у Вхідні» — для скасованих/архівних
  (`taskStore.restoreFromHistory()`); виконані й активні не мають —
  перших повертати нема сенсу, другі й так активні.
- **Розклад підзадач** під рядком (лише коли підзадачі є) —
  `Підзадачі · N / M виконано` + сам список: виконані з позначкою
  `✓` і закреслені, решта з `○`. Лише для огляду (без чекбоксів) —
  редагувати підзадачі можна на `/task/:id`.
  `getSubtasksByTaskIds()` повертає `Map task_id → Subtask[]`,
  `HistoryList` роздає кожному рядку його групу.
- **Звіт за період** — пресети (сьогодні / поточний і минулий
  тиждень / поточний і минулий місяць, тиждень із понеділка) чи
  довільний діапазон (два `<input type="date">`) — фільтрує
  завершені за `completed_at ?? cancelled_at ?? updated_at`,
  лічильники (`✅ N · 🚫 M` + `🔵 K` у режимі «Усі») над списком.
...
повертається до звичайної колонки. Прилипає при цьому **вся `.nav`**
(`position: sticky` саме на ній у цьому брейкпоінті), а не сама
смужка: `.nav` без панелі заввишки рівно зі смужкою, тож sticky на
смужці не мав куди «липнути» — вона прокручувалась геть разом із
`.nav` (баг, знайдений користувачем). `sticky` не створює containing
block для `fixed`-нащадків, тож `.nav__panel`/`.nav__overlay` і далі
кріпляться до в'юпорта. `.nav__brand` — простий текстовий
...
`insert` у `tasks` (+ `subtasks` з `position` за порядком від ШІ,
якщо є) тим самим `service_role`-клієнтом. У відповідь —
`sendMessage` з підтвердженням і списком підзадач, якщо вони є.
...
**Команда `/report_range`** — просто друкує синтаксис довільного
діапазону (`RANGE_HELP_TEXT` у коді): `/report ДД-ММ-РРРР ДД-ММ-РРРР`
або одна дата `/report ДД-ММ-РРРР`. Раніше вона слала `force_reply`
і ловила відповідь через `message.reply_to_message.text` — але
Telegram-клієнт «залипає» на такому запиті: поле вводу лишається
прив'язаним до нього навіть після перезаходу в чат, і наступне
повідомлення користувача йде як `reply`, а не як нова задача (саме
це й дратувало користувача — 2026-09). **Тому бот принципово не
використовує `reply_markup` взагалі** (ні `force_reply`, ні
клавіатур) — `sendMessage()` більше не приймає таких опцій. Кожна
команда самодостатня: або одразу робить дію, або друкує синтаксис,
який треба набрати звичайним повідомленням.
...
- 🔴 **Термінові** — `status = "urgent"`, якщо дедлайн задачі не
  потрапив у жоден з блоків вище (чи дедлайну нема); задача зі
  статусом `urgent`, що *й так* потрапила в один із блоків за датою,
  лишається там (без дублювання в двох місцях);
- 🗒️ **Інші задачі** — решта задач із «Вхідні»/«Задачі», що не
  підпали під жоден критерій вище і вже актуальні: без дедлайну або
  з дедлайном, чий період уже почався (весь список повністю, а не
  лише найтерміновіше — за прямим проханням користувача).

Задачі, чий період ще попереду (початок пізніше за завтра) і які не
термінові, у дайджест не потрапляють узагалі (`bucketOf()` повертає
`"future"`, і `buildDigestMessage` їх пропускає) — з'являться в
дайджесті того дня, коли їхній період настане.
...
> **Стан на 2026-09-01:** `CRON_SECRET` **не заданий** (прибраний з
> секретів функції). Раніше він з'явився під час налаштування
> `feed-poll`, але cron-завдання `daily-reminder-9am-kyiv` і далі
> слало старий рядок-заглушку в `x-cron-secret` → функція відповідала
> `403 forbidden`, і ранкова розсилка мовчки зникла (перший
> пропущений день — 2026-09-01). `if (CRON_SECRET)` у коді функції:
> порожній секрет = перевірка пропускається, тож зараз виклик просто
> проходить. Хочеш повернути захист — задай `CRON_SECRET` **і** в
> тому ж кроці переплануй cron із тим самим значенням у заголовку
> (`cron.schedule('daily-reminder-9am-kyiv', …)`), інакше знову 403.
...
> **Статус: вимкнено** (`js/config.js` → `FEATURES.feed = false`, за
> проханням користувача — поки не потрібно). Обидві сторінки зникли
> з навігації, прямий перехід на `/feed` / `/sources` редіректить на
> «Вхідні» (`ROUTES` у [router.js](../js/router.js) фільтрується за
> прапорцем — `matchRoute` їх не знаходить; `AccountMenu.js` ховає
> «Джерела»). Щогодинний `feed-poll` знято окремою міграцією
> [20260901000000_pause_feed_poll_cron.sql](../supabase/migrations/20260901000000_pause_feed_poll_cron.sql)
> (щоб не витрачати платний Apify для Instagram). Код усіх сторінок/
> сторів/Edge Functions і таблиці `sources`/`feed_items` з даними —
> недоторкані. **Повернути:** `FEATURES.feed = true` + повторно
> виконати
> [20260831000000_setup_feed_poll_cron.sql](../supabase/migrations/20260831000000_setup_feed_poll_cron.sql).
>
> Решта цього розділу описує фічу як вона працює, коли ввімкнена.

Дві сторінки під один сценарій: «Джерела» (`/sources`) —
...
5. Наповнити стрічку постами — двома способами, не взаємовиключними:
   - **`feed-poll` (вбудовано)** — для платформ YouTube/Telegram/RSS
     (безкоштовно) і Instagram (платно, через Apify), дивись
     підрозділ нижче;
   - **зовнішній парсер** (Apify/Firecrawl/будь-що інше, що вміє
     надіслати POST) — для решти (Threads/Reddit/Twitter,
     `feed-poll` їх не підтримує), чи разово вручну — слати
     результат на
...
за розкладом (pg_cron, щогодини) сама читає всю таблицю `sources` і
для кожного рядка тягне свіжі пости, кожна платформа своїм способом
([_shared/feedParse.ts](../supabase/functions/_shared/feedParse.ts)):

- `rss` — `handle` і є URL фіда напряму (має бути `http(s)://...`),
  фід — звичайний RSS/Atom XML.
...
- `telegram` — `fetchTelegramEntries()`: офіційний легкий веб-перегляд
  публічного каналу (`https://t.me/s/<channel>`, `handle` без `@`/
  `https://t.me/`), розбір власної HTML-розмітки (не RSS). Публічний
  RSSHub-міст (перший варіант, що напрошувався) відпав — тепер стоїть
  за Cloudflare-захистом від ботів, недоступний з Edge Function.
- `instagram` — `fetchInstagramEntries()`: **платний** виклик Apify
  (`apify.com`, актор `apify/instagram-post-scraper`,
  `run-sync-get-dataset-items` — стартує й одразу повертає результат,
  без окремого опитування статусу), потребує секрет
  `APIFY_API_TOKEN`. Безкоштовного RSS для Instagram нема.
  Перевіряється **раз на 2 години** (парні UTC-години), не щогодини —
  профіль однаково не постить щогодини, а Apify бере гроші за кожен
  скрапінг незалежно від того, чи пост уже бачили (дедуп у
  feed-webhook рахується вже ПІСЛЯ оплати). Free-план Apify — $5
  кредиту/міс, актор коштує ≈$0.0027/пост — за замовчуванням (5
  постів × 12 перевірок/добу) укладається в безкоштовний ліміт із
  запасом.
- `threads`/`reddit`/`twitter` — **не підтримані** (немає
  безкоштовного RSS і не підключено Apify-актора),
  `resolveFeedUrl()` повертає `null`, джерело просто пропускається
  (потрапляє в `skipped` у відповіді функції); ці платформи — лише
  через зовнішній платний скрапер і ручний `feed-webhook`.

Розпарсені записи пересилаються батчем у сам `feed-webhook`
...
2. `CRON_SECRET` — той самий, що й для `daily-reminder`, якщо він
   ...
   supabase secrets set CRON_SECRET=<той_самий_рядок>
   ```
3. Задля Instagram — зареєструватись на [apify.com](https://apify.com)
   (безкоштовний план, картка не потрібна), у профілі створити API-
   токен і задати секрет:
   ```bash
   supabase secrets set APIFY_API_TOKEN=<токен_з_Apify>
   ```
   Без цього кроку Instagram-джерела просто мовчки пропускаються
   (`skipped`) — решта платформ (YouTube/Telegram/RSS) працюють і без
   нього.
...
5. Виконати міграцію
   [20260831000000_setup_feed_poll_cron.sql](../supabase/migrations/20260831000000_setup_feed_poll_cron.sql)
   у Supabase SQL Editor (**виконано** — 2026-08-31) — **перед
   запуском** замінити
   `REPLACE_WITH_YOUR_CRON_SECRET` у самому SQL-файлі (в редакторі,
   не в git) на той самий рядок, що й у кроці 2.
6. Перевірити вручну, не чекаючи на початок години (підставивши свій
   `CRON_SECRET`):
   ```bash
...
│   │   │   └── feedParse.ts         — RSS/Atom-парсер (YouTube/RSS) +
│   │   │                         Telegram-скрапер (t.me/s/...) + Apify-виклик
│   │   │                         (Instagram), для feed-poll/
│   │   ├── telegram-webhook/
...
│       │                          (виконано; знято міграцією нижче)
│       ├── 20260901000000_pause_feed_poll_cron.sql
│       │                          — знімає feed-poll-cron (Стрічку вимкнено,
│       │                          FEATURES.feed); виконано 2026-09-01
│       └── 20260901010000_add_position_to_subtasks.sql
│                                  — subtasks.position (порядок «↑»/«↓»);
│                                  потребує виконання
├── docs/
...
| `/board`                | Дошка                  | лише авторизованим | Kanban: задачі списку «Задачі» (`list: next`) колонками статусів, drag-and-drop |
...
| `/feed`                | Стрічка                | лише авторизованим · **вимкнено** (`FEATURES.feed`) | картки постів: В Inbox / Пропустити / Відкрити |
| `/history`             | Історія                | лише авторизованим | автоперенесені виконані/скасовані (о 22:30) + звіт за період |
| `/sources`             | Джерела                | лише авторизованим, поза головним меню (AccountMenu.js) · **вимкнено** (`FEATURES.feed`) | форма додавання + список підписок, id для вебхука |
...
**Прапорці фіч.** Маршрут із полем `feature: "<ключ>"` існує лише
коли `FEATURES["<ключ>"] === true` ([js/config.js](../js/config.js)):
`ALL_ROUTES` фільтрується в `ROUTES` ще до будь-якого матчингу, тож
вимкнений маршрут для роутера просто не існує — і в меню його нема
(`getRoutes()`), і прямий перехід на нього `matchRoute()` не
знаходить → редірект на «Вхідні». Зараз так вимкнено `feed`
(`/feed` + `/sources`, розділ 1 «Джерела і Стрічка»); компоненти,
які самі малюють посилання на такі сторінки (напр. `AccountMenu.js`
→ «Джерела»), додатково звіряються з тим самим `FEATURES`.
...
задачі списку «Задачі» одним запитом (`getAllTasks()` — `list:
"next"` лише) і сам розкладає
...
### Адаптив (мобільні брейкпоінти)

Окремого файлу-«responsive» немає — `@media`-правила лежать у тому
самому компонентному CSS, що й базові стилі елемента. Спільні
пороги:

- **`max-width: 959px`** — навігація ([css/nav.css](../css/nav.css)):
  бічна панель ховається за бургером, `body` з флекс-рядка стає
  колонкою ([css/style.css](../css/style.css), той самий поріг).
- **`max-width: 960px` / `620px`** — дошка
  ([css/board.css](../css/board.css)): 6 колонок статусів → 2 → 1
  (одна під одною), без горизонтального скролу.
- **`max-width: 600px`** — телефон: менші внутрішні відступи
  сторінки й карток, блоки «Матеріали»/«Стрічка»/«Історія»/
  «Кошик»/«Інтеграції», кнопки-пігулки на всю ширину
  (`page.css`, `task-detail.css`, `task-form.css`, `feed.css`,
  `history.css`, `trash.css`, `integrations.css`).
- **`max-width: 560px`** — картка задачі
  ([css/task-card.css](../css/task-card.css)): «Статус»+«Список» в
  одну колонку, поля дедлайну/повторення на всю ширину (у ряд
  `<input type="date">` + `<select>` не влазять і виїжджають за
  екран), чекбокс/кнопки рівняються по верху багаторядкової назви.
- **`max-width: 480px`** — форма «Джерела»
  ([css/sources.css](../css/sources.css)): поля на всю ширину.

Довгий текст без пробілів (вставлене посилання в назві/нотатці/
пості) скрізь має `overflow-wrap: anywhere` — інакше розпирав картку
ширше за екран (реальний баг на телефоні). `body` тримає
`overflow-x: hidden` як остання лінія оборони.
...
- **«Джерела» / «Стрічка»** — **вимкнено** за проханням користувача
  (`js/config.js` → `FEATURES.feed = false`, cron знято міграцією
  `20260901000000_pause_feed_poll_cron.sql`; повернути —
  розділ 1, підрозділ «Джерела і Стрічка»). Фіча робоча: приймальна
  частина (SQL-міграція, `FEED_WEBHOOK_SECRET`, деплой `feed-webhook`)
  й автоматичний збір (`feed-poll` + `pg_cron`) підтверджені живими
  тестами 2026-08-31: YouTube/Telegram/RSS — 32/32 постів збережено.
  Instagram — через Apify (`APIFY_API_TOKEN`), лишалось задати секрет
  і перевірити на реальному джерелі. Threads/Reddit/Twitter — і далі
  лише вручну/через сторонній парсер.
