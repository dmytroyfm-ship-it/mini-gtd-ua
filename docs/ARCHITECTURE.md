...
часом перевіряє `tasks` (активні, не виконані) й будує повний
список задач на день для кожного `user_id`, згрупований по чотирьох
блоках (не лише лічильники):
...
Один SQL-запит бере все одразу: `due_date <= завтра OR status =
'urgent'`, потім кожна задача розкладається по блоку в JS
(`bucketOf()`). Немає жодної задачі, що підпадає під ці критерії, чи
нема прив'язки Telegram — мовчки нічого не надсилає. Повідомлення
понад 3800 символів акуратно обрізається (Telegram API мовчки
відхиляє все, що довше 4096, — для особистого використання навряд
чи трапиться, але про всяк випадок).
