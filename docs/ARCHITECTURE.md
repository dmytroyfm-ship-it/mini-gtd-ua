...
`.page--wide` (ширший контент, `--board-width: 2200px`, замість
звичайних 768px) — усі 6 колонок статусів показані одразу, без
спойлера, і поруч їм тісно в стандартній ширині сторінки; коли й
2200px не вистачає (менші екрани) — ряд колонок сам займає всю
ширину сторінки й прокручується (`overflow-x: auto`).
[js/pages/board.js](../js/pages/board.js) бере
