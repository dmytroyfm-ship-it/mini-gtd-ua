...
- **Сторінки `/list/next`, `/list/someday`, `/list/read_watch`,
  `/list/archive`** — досі заглушки. Dropdown «Список» у картці
  задачі вже вміє переносити задачу в будь-який із них (колонка
  `list`), але побачити її там можна поки лише в базі — самі
  сторінки для цих списків не показують задачі, як «Вхідні».
