...
спецсимволом). Кнопка-лупа праворуч у `Nav.js` (перед
`.nav__account-slot`) розкриває панель з полем — клік поза нею чи
Escape закриває; під час набору (дебаунс `DEBOUNCE_MS` — не смикати
базу на кожну літеру) з'являється випадний список до
`SUGGESTION_LIMIT` збігів, клік по одному одразу відкриває
