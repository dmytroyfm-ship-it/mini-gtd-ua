...
аргументу — найчастіший запит), `місяць` — поточний, `весь` — без
обмежень, `ДД-ММ-РРРР ДД-ММ-РРРР` — довільний діапазон, чи рівно
одна дата `ДД-ММ-РРРР` — усе ДО цієї дати, без нижньої межі (той
самий принцип, що й поле «По» без «З» на сторінці «Історія»). Без
...
`/report_all` (`COMMAND_TO_ARGS` у коді) — та сама
